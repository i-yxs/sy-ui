...
### baseData
> 开发项目中项目通常会存在一些稳定的数据（指大部分时间内都不会变动，但是需要从线上服务器请求的数据，比如数据字典、省市区数据等等），这些数据推荐在用户第一次打开页面时，请求并缓存到vuex，不推荐每次需要时就请求一次，这会极大的增加服务器压力。且本组件库引入了链式路径访问store内数据的方案，比如设置sy-picker-tree组件的options的值时，可以指定options为链式路径字符串，组件内部会自动从store内获取数据，该写法可以极大的降低重复代码和后期维护成本<br>
